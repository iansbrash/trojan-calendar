...
const getMyUscCookiesDuo1 = async (username, password) => {
    ...
    // /account/login
    const AccountLoginResponse = await tryCatchWrapper(() => axios({
        method: 'get',
        url:  'https://myrecsports.usc.edu/Account/Login', // 'https://api.usc.edu/auth/trojancheck/InitiateSingleSignOn?returnurl=https://trojancheck.usc.edu/sso',
        headers: genHeaders(allCookies),
        maxRedirects: 0,
        validateStatus: VS,
    }), "AccountLoginResponse");

    allCookies = accumulateCookies(allCookies, returnParsedCookies(AccountLoginResponse.headers['set-cookie']))


    const rvt = getValueByDelimiters(AccountLoginResponse.data, '<input name="__RequestVerificationToken" type="hidden" value="', '" />')


    // // https://myrecsports.usc.edu/Account/ExternalLogin?ReturnUrl=%2Fbooking%2Fcd93ade2-af9d-4e5f-84e0-06e10711b5ce
    const InitialPOSTForShibStateResponse = await tryCatchWrapper(() => axios({
        method: 'post',
        url:  'https://myrecsports.usc.edu/Account/ExternalLogin?ReturnUrl=%2F', // 'https://api.usc.edu/auth/trojancheck/InitiateSingleSignOn?returnurl=https://trojancheck.usc.edu/sso',
        headers: genHeaders(allCookies),
        maxRedirects: 0,
        validateStatus: VS,
        data: qs.stringify({
    ...
    }), "InitialPOSTForShibStateResponse");


    allCookies = accumulateCookies(allCookies, returnParsedCookies(InitialPOSTForShibStateResponse.headers['set-cookie']))


    const ShibResponse = await tryCatchWrapper(() => axios({
        method: 'get',
        url:  `https://myrecsports.usc.edu${InitialPOSTForShibStateResponse.headers['location']}`,
        headers: genHeaders(allCookies),
        maxRedirects: 0,
        validateStatus: VS,
    }), "InitialPOSTForShibStateResponse");

    allCookies = accumulateCookies(allCookies, returnParsedCookies(ShibResponse.headers['set-cookie']))
    ...
    let Res2 = await tryCatchWrapper(() => axios({
        method: 'get',
        url: ShibResponse.headers.location,
        headers: genHeaders(allCookies),
        maxRedirects: 0,
        validateStatus: VS
    }), "Res2");
